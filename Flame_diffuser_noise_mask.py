...
def load_img(image, path, mask, noise_std):
    ...
    # image.save(os.path.join(outpath, f"{image_name}_{i}.jpg"))
    ...
    # =======================================================
    # Comment this section to turn masks into binary (b/w)
    mask[:, :, 0] = mask[:, :, 0] * R
    mask[:, :, 1] = mask[:, :, 1] * G
    mask[:, :, 2] = mask[:, :, 2] * B
    # =======================================================
    ...
    # void = np.zeros_like(image)
    
    # To tensor
    image = np.array(mask).astype(np.float32) / 255.0
...
if __name__ == "__main__":
    ...
    num_sample = 10
    noise_std = 0.1 #random.uniform(0, 0.1)
    ...
    outpath = f'./exp/outputs/noise_test_{noise_std}/'
            ...
            with model.ema_scope():
                ...
                while i < num_sample:
                    try:
                        ...
                        # mask_file = mask_path+'611_mask.jpg'
                        ...
                        # opt.strength = 0.6 #random.uniform(0.2, 0.7)
                        # opt.ddim_steps = int(10/(opt.strength))
                        # noise_std = 0.01 #random.uniform(0, 0.1)
                        ...
                        # mask_save_name = f"{image_name}_{mask_name}_{i}.png"
