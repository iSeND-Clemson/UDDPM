...
def load_img(image_name, path, noise_std):
    ...
    mask = Image.open(mask_path+image_name+'.jpg').convert("L")
    ...
    # Map color
    mask[:, :, 0] = mask[:, :, 0] * np.random.uniform(0.78, 1)
    mask[:, :, 1] = mask[:, :, 1] * np.random.uniform(0.2, 0.4)
    mask[:, :, 2] = mask[:, :, 2] * np.random.uniform(0, 0.2)
                ...
                with model.ema_scope():
                    ...
                    while i < num_sample:
                        try:
                            ...
                            opt.init_img = img_path+img_file
                            # opt.init_img = "./test_img/image_skeleton_dia.png"
                            # prompt_list = [" ", "polygon", "nano materials", "dendrite", "lab sample", "chemistry", "biology", "encryption", "code"]
                            ...
                            opt.prompt = "photo realistic, wildfire in the forest, flame and fire in the smoke, high resolution"
                            opt.scale = random.randint(5, 15)
                            opt.strength = random.uniform(0.4, 0.60)
                                ...
                                for prompts in tqdm(data, desc="data"):
                                    ...
                                    if not opt.skip_save:
                                        for x_sample in x_samples:
                                            ...
                                            image.save(
                                                os.path.join(sample_path, f"{image_name}_{base_count:04}.png"))
                                            cv2.imwrite(os.path.join(sample_path, f"{image_name}_mask.png"), mask)
                                            base_count += 1
                                    all_samples.append(x_samples)
