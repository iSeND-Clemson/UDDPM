...
def load_img(path, noise_std, image_name, save_path):
    ...
    image = image + np.random.normal(0, noise_std, size=(w,h,z))
    plt.imshow(image)
    image = (image - np.min(image))/(np.max(image)-np.min(image)) *255
    
    if save_path:
        cv2.imwrite(f'{save_path}{image_name}.png', image)
            ...
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    for i in range(num_sample):
                        ...
                        opt.prompt = " "
                        opt.scale = random.randint(5, 200)
                        # opt.strength = random.uniform(0, 0.6)
                        opt.strength = 20/opt.scale
                        opt.ddim_steps = int(11/opt.strength)
                        if opt.ddim_steps>=100: opt.ddim_steps=100
                        noise_std = random.uniform(0, 0.5)
                        ...
                        assert os.path.isfile(opt.init_img)
                        init_image = load_img(opt.init_img, noise_std, image_name).to(device)
                        init_image = repeat(init_image, '1 ... -> b ...', b=batch_size)
                        init_latent = model.get_first_stage_encoding(model.encode_first_stage(init_image))  # move to latent space
